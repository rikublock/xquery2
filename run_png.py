...
def main() -> int:
    ...
    contract_address = Web3.toChecksumAddress(png_factory.address)
    ...
    indexer_cls = xquery.event.EventIndexerExchangePangolin

    # create an event filter
    event_filter = xquery.event.EventFilterExchangePangolin(
        w3=w3,
    ...
    with xquery.controller.Controller(w3=w3, db=db, indexer_cls=indexer_cls, num_workers=int(C["XQ_NUM_WORKERS"])) as c:
        start_block = max(png_factory.from_block, state.block_number + 1)
